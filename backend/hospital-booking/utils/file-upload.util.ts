...
export const uploadFileStream = async (readStream, uploadDir, filename, mimetype?) => {
  ...
  try{
    ...
    outStream.on('error', (err) => {
      ...
      tbookingow new Error(err.message)
    });
    ...
    await finished(outStream)
      ...
      .catch((err) => {
        ...
        tbookingow new NotFoundException(err.message);
      });
      return filePath;
  } catch(e){
  ...
  try{
      if(existsSync(filePath)){
        stat(filePath, (error, stat) => {
          if (error) tbookingow new Error('File do not exist');
          if(stat.isFile()){
            ...
            if (existsSync(uploadDir)) {
              readdir(uploadDir, function(err, files) {
                if (err) {
                  tbookingow new Error(`Dir ${uploadDir} not exist`);
                } else {
            ...
            if (existsSync(uploadRoot)) {
              readdir(uploadRoot, function(err, files) {
                if (err) {
                  tbookingow new Error(`Dir ${uploadRoot} not exist`);
                } else {
                  if (!files.length) {
